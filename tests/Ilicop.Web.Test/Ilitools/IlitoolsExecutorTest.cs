...
namespace Geowerkstatt.Ilicop.Web.Ilitools
{
    ...
    public class IlitoolsExecutorTest
    {
        ...
        public void CreateIli2GpkgImportCommand()
        {
            ...
            var expected = $"-jar \"{ilitoolsEnvironment.Ili2GpkgPath}\" --update --disableValidation --skipReferenceErrors --skipGeometryErrors --importTid --importBid --dataset \"{request.Dataset}\" --dbfile \"{request.DbFilePath}\" --modeldir \"{ilitoolsEnvironment.ModelRepositoryDir}\" --metaConfig \"ilidata:{request.Profile.Id}\" \"{request.FilePath}\"";
            Assert.AreEqual(expected, command);
        }
